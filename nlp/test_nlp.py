...
def test_stem():
    nlp = FooNLP()
    assert nlp.stem('worked at happening places') == 'work at happen place'
...
def test_tfidf():
    nlp = FooNLP(FooModel(embedding=TfidfVectorizer))
    sentences = ['The indian life of the indian pi', 'The life and pain of the french fiancée', 'my life my death my pain']
    (m, h) = nlp.make_embeddings(sentences)
...
def test_stanford_tfidf():
    nlp = FooNLP(model=FooModel(embedding=TfidfVectorizer))
