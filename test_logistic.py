...
def test_evalPartialDeriv2():
    ...
    log.warn('init func: %s, training size: %d' %(str(f),len(trainingMatrix)))
    ...
    for theta in ts:
        ...
        print ('deriv %s of %s = %f'%(str(f),str(theta),pd))
        pds.append(pd)
    print ('done')
...
# test hack
    ...
    gs = grad_descent4(f,costF,trainingMatrix,yArr,step=0.1,loop_limit=1000,step_limit=0.00000000001)    
